...
def save_ground_states_and_labels(save_dir: str, num_points: int, uniform: bool, epsilon: bool, nqubits: int, ham: str, filename_prefix: str):
    # Create the directory if it does not exist
    os.makedirs(save_dir, exist_ok=True)

    # Generate the ground state and labels
    gs_list, labels_list, j_list = generate_gs(num_points, uniform, epsilon, nqubits, ham)

    # Save data
    data = [{"j1": j[0], "j2": j[1], "ground_state": gs} for j, gs in zip(j_list, gs_list)]
    labels = labels_list.tolist()

    data_filename = f"{filename_prefix}_data.pkl"
    labels_filename = f"{filename_prefix}_labels.pkl"

    with open(os.path.join(save_dir, data_filename), 'wb') as f:
        pickle.dump(data, f)

    with open(os.path.join(save_dir, labels_filename), 'wb') as f:
        pickle.dump(labels, f)

    print(f"Data saved to {data_filename} and {labels_filename}")

if __name__ == "__main__":
    nqubits = 8

    for ham in ["gch"]: # "gch" (Generalized Cluster Hamiltonian) 
        save_dir = f"../data/phase_detection_{ham}_nq_{nqubits}"

        num_train_points = 50
        train_filename_prefix = f"ground_nq_{nqubits}_train_{num_train_points}_{ham}_train"
        save_ground_states_and_labels(save_dir, num_train_points, uniform=False, epsilon=False, nqubits=nqubits, ham=ham, filename_prefix=train_filename_prefix)

        num_test_points = 1000
        test_filename_prefix = f"ground_nq_{nqubits}_test_{num_test_points}_{ham}_test"
        save_ground_states_and_labels(save_dir, num_test_points, uniform=False, epsilon=False, nqubits=nqubits, ham=ham, filename_prefix=test_filename_prefix)